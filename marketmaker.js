        ...
        case "orders":
            ...
            orders.forEach((order) => {
                ...
                if (fillable.fillable) {
                    ...
                    ["sending order already", "badprice"].includes(
                        fillable.reason
                    )
                ) {
...
function isOrderFillable(order) {
    ...
    const price = order[4];
    const sellCurrency =
        side === "s" ? market.quoteAsset.symbol : market.baseAsset.symbol;
    const sellDecimals =
        side === "s" ? market.quoteAsset.decimals : market.baseAsset.decimals;
...
function genQuote(chainId, marketId, side, baseQuantity) {
    ...
    if (side === "b") {
        quoteQuantity =
            baseQuantity * primaryPrice * (1 + SPREAD) + market.quoteFee;
    } else if (side === "s") {
        quoteQuantity =
            (baseQuantity - market.baseFee) * primaryPrice * (1 - SPREAD);
    }
...
async function sendFillRequest(orderreceipt, accountId) {
    ...
    if (side === "b") {
        ...
        sellQuantity = (baseQuantity * 1.0001).toFixed(
            market.baseAsset.decimals
        );
        buyQuantity = (quote.quoteQuantity * 0.9999).toFixed(
        ...
        // Add 1 bip to to protect against rounding errors
        sellQuantity = (quote.quoteQuantity * 1.0001).toFixed(
            market.quoteAsset.decimals
        );
        buyQuantity = (baseQuantity * 0.9999).toFixed(
            market.baseAsset.decimals
        );
    }
    ...
    const sellQuantityPacked =
        zksync.utils.closestPackableTransactionAmount(sellQuantityParsed);
    const tokenRatio = {};
    ...
    const fillOrder = await WALLETS[accountId].syncWallet.getOrder(
        orderDetails
    );
    ...
    for (const marketId in pairs) {
        ...
        for (let i = 1; i <= splits; i++) {
            const buyPrice =
                ...
                (1 -
                    mmConfig.minSpread -
                    (mmConfig.slippageRate * maxBuySize * i) / splits);
            const sellPrice =
                midPrice *
                (1 +
                    mmConfig.minSpread +
                    (mmConfig.slippageRate * maxSellSize * i) / splits);
        ...
        const msg = {
            op: "indicateliq2",
            args: [CHAIN_ID, marketId, liquidity],
        };
        try {
            zigzagws.send(JSON.stringify(msg));
            console.log(
                "Sucessfully indicated liquidity",
                util.inspect(msg, {
                    showHidden: false,
                    depth: null,
                    colors: true,
...
async function afterFill(chainId, orderId, wallet) {
    ...
    const oldbuyTokenParsed = ethers.BigNumber.from(
        account_state[order.buySymbol]
    );
    const oldsellTokenParsed = ethers.BigNumber.from(
        account_state[order.sellSymbol]
    );
    account_state[order.buySymbol] = oldbuyTokenParsed
        .add(buyTokenParsed)
        .toString();
    account_state[order.sellSymbol] = oldsellTokenParsed
        .sub(sellTokenParsed)
        .toString();
    ...
    try {
        ...
        await Promise.all(
            Object.keys(balance).map(async (token) => {
                const price = await syncProvider.getTokenPrice(
                    token.toString()
                );
                const tokenNumber = await syncProvider.tokenSet.formatToken(
                    token,
